...
class Career_model extends CI_Model{
 protected $table_name = "tbl_careerenquiry";
 //protected $enquiry_table = "tbl_careerenquiry";
...
function selectdata($column, $cond, $search,$start="",$limit=""){
 ...
 function deletedata($ids) {
 	if(is_array($ids) && count($ids)) {
		$this->db->where_in('cid',implode(",",$ids));
	...
	function applicants($column, $cond, $search, $start = "", $limit = "")
	{
		$limitcond = '';
		$condStrArr = array();
		if (is_array($cond) && count($cond)) {
		...
		$sql = "select $column from " . $this->enquiry_table . " where 1=1  $condStr $search $limitcond";
		$query_result = $this->db->query($sql);
		$result = $query_result->result();
		return $result;
	}
