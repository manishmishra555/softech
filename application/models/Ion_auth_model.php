...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Abhishek
	 **/
	public function adminlogin($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$this->trigger_events('extra_where');
        /*$this->db->select($this->tables['users'].'.'.$this->identity_column.', '.$this->tables['users'].'.'.'email'.', '.$this->tables['users'].'.'.'id, '.$this->tables['users'].'.'.'password, '.$this->tables['users'].'.'.'active, '.$this->tables['users'].'.'.'last_login,'.$this->tables['users_groups'].'.'.$this->join['groups'])
						  ->join($this->tables['users_groups'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.'.'id')						  
		                  ->where($this->tables['users_groups'].'.'.$this->join['groups'], '1')
  		                  ->where($this->tables['users'].'.'.$this->identity_column, $identity)						 
						  ->limit(1)
		    			  ->order_by($this->tables['users'].'.'.'id', 'desc')
		                  ->get($this->tables['users']);
						  echo $this->db->last_query(); die;*/
		$query = $this->db->select($this->tables['users'].'.'.$this->identity_column.', '.$this->tables['users'].'.'.'email'.', '.$this->tables['users'].'.'.'id, '.$this->tables['users'].'.'.'password, '.$this->tables['users'].'.'.'active, '.$this->tables['users'].'.'.'last_login,'.$this->tables['users_groups'].'.'.$this->join['groups'])
						  ->join($this->tables['users_groups'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.'.'id')						  
  		                  ->where($this->tables['users'].'.'.$this->identity_column, $identity)						 
						  ->limit(1)
		    			  ->order_by($this->tables['users'].'.'.'id', 'desc')
...
/**
	 ...
	 * @author Abhishek
	 **/
	public function memberlogin($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->tables['users'].'.'.$this->identity_column.', '.$this->tables['users'].'.'.'email'.', '.$this->tables['users'].'.'.'id, '.$this->tables['users'].'.'.'password, '.$this->tables['users'].'.'.'active, '.$this->tables['users'].'.'.'last_login,'.$this->tables['users_groups'].'.'.$this->join['groups'])
						  ->join($this->tables['users_groups'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.'.'id')						  
		                  ->where($this->tables['users_groups'].'.'.$this->join['groups'], '2')
  		                  ->where($this->tables['users'].'.'.$this->identity_column, $identity)						 
						  ->limit(1)
		    			  ->order_by($this->tables['users'].'.'.'id', 'desc')
		                  ->get($this->tables['users']);
	...
	*/
	 ...
	 * @author Ben Edmunds
	 **/
	public function users($groups = NULL,$location = NULL,$page=NULL,$perpage=NULL)
	{
		...
		$this->trigger_events('extra_where');
		//filter by location if passed
 		if(isset($location)){
 			$this->db->where_in($this->tables['users'].'.locations',$location);
		 }
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,'.$this->tables['groups'].'.default_page')
	...
	**/
	 ...
	 **/
	public function permissions($id = NULL,$modulename=NULL)
	{
		...
		{
			//echo $id;die;
			$this->db->where($this->tables['permissions'].'.group_id', $id);
		}
		if (isset($modulename))
		{
			//echo $id;die;
			$this->db->where($this->tables['permissions'].'.modulename', $modulename);
		}
  		$this->db->order_by('id', 'desc');
