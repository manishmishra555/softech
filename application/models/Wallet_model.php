...
class Wallet_model extends CI_Model{
 protected $table_name = "tbl_wallet";
 protected $transaction_table = "tbl_wallet_transactions";
...
function selectdata($column, $cond, $search,$start="",$limit=""){
 ...
 function selecttransaction($column, $cond, $search,$start="",$limit=""){
	$limitcond='';
	$condStrArr = array();
	 if(is_array($cond) && count($cond)){
		 foreach($cond as $ind=>$val) {
	...
	if($limit!='' || $start!=''){
	 ...
	 $sql = "select $column from ".$this->transaction_table." where 1=1  $condStr $search $limitcond";
	$query_result=$this->db->query($sql);
	$result = $query_result->result();
	return $result;
}
