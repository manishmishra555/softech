...
class Meesho extends Admin_Controller
{
    ...
    function __construct()
    {
        parent::__construct();
 		...
 		$this->load->model('meesho_model');
 		$this->load->model('category_model');
 		$this->load->model('brand_model');
     }
     public function index()
    {
        $this->load->view('sysadmin/meesho');
    }
    public function products()
    {
		$this->load->library('pagination');
        $this->data['page_title'] = 'Meesho Products';
		//Gallery
		...
		$this->data['post_pics'] = $this->gallery->getMediaGallery('post_pics', $attr);
   		...
   		$this->data['total_record'] = $this->product_model->selectdata("COUNT(*) as totalrecords",array("platform" => "meesho"),"ORDER BY pid DESC");
 		$this->load->library('pagination');
 		$config['base_url'] = MAINSITE_MADMIN_URL.'meesho/products/page/';
		...
		$this->data['product'] = $this->product_model->selectdata("*",array("platform" => "meesho"),'ORDER BY pid DESC',$page,$config["per_page"]);
        ...
        $this->render('sysadmin/meesho/list_products');
	}
   ...
   public function orders()
    {
		$this->load->library('pagination');
        $this->data['page_title'] = 'Meesho Orders';
		//Gallery
		$this->load->library('gallery');
		$attr = array('class' => 'btn btn-primary btn-block btn-large');
		$this->data['post_pics'] = $this->gallery->getMediaGallery('post_pics', $attr);
		
   		$this->data['total_record'] = $this->meesho_model->selectdata("COUNT(*) as totalrecords",array(),"ORDER BY id DESC");
 		$this->load->library('pagination');
 		$config['base_url'] = MAINSITE_MADMIN_URL.'meesho/orders/page/';
		$config['total_rows'] = $this->data['total_record'][0]->totalrecords;
 		$config['per_page'] = RECORD_PER_PAGE;
		$config["uri_segment"] = PAGINATION_URI_SEGMENT;
 		$config['attributes'] = array('class' => 'page-link');
		...
		$page = ($this->uri->segment(PAGINATION_URI_SEGMENT)) ? $this->uri->segment(PAGINATION_URI_SEGMENT) : 0;
		$this->data['total_pages']  = $page;
		$this->data['order'] = $this->meesho_model->selectdata("*",array(),'ORDER BY id DESC',$page,$config["per_page"]);

		$this->data['pageing_link'] = $this->pagination->create_links();
		$this->data['extracss'] = '<link rel="stylesheet" href="' . ADMIN_ASSETS_PATH . 'vendors/bower_components/select2/dist/css/select2.min.css">';
		$this->data['extrajs'] = '<script src="' . ADMIN_ASSETS_PATH . 'vendors/bower_components/select2/dist/js/select2.full.min.js"></script>';
        $this->render('sysadmin/meesho/orders');
	}
	...
	public function createorder(){
		...
		$this->data['extrajs'] = '<script src="' . ADMIN_ASSETS_PATH . 'vendors/bower_components/select2/dist/js/select2.full.min.js"></script>';
		// echo "<pre>";print_r($this->data['extrajs']);die;

		$this->load->view('sysadmin/meesho/createorder', $this->data);
	}
