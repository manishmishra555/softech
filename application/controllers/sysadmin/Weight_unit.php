...
class Weight_unit extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['page_title'] = 'Country';
   		...
   		$this->data['total_record'] = $this->weight_unit_model->selectdata("*",array(),"ORDER BY id DESC");
 		$this->load->library('pagination');
		...
		$this->data['country'] = $this->weight_unit_model->selectdata("*",array(),'ORDER BY id DESC',$page,$config["per_page"]);
	...
	public function page()
   {
    ...
    public function create()
    {
        ...
        else
        {
			...
			$this->weight_unit_model->insertdata($column);
    ...
    public function edit($country_id = NULL)
    {
        ...
        if($this->form_validation->run() === FALSE)
        {
			$country =$this->weight_unit_model->selectdata("*",array('id'=>$country_id),"ORDER BY id DESC");
            if(count($country)>0)
            {
        ...
        else
        {
            ...
            $this->weight_unit_model->updatedata($column,array('id'=>$country_id));
...
public function status()
	{   
		...
		$conditions=array();
		$conditions['id']=$id;
		$this->weight_unit_model->updatedata($columns,$conditions);
	...
	{   
	    ...
	    $deleted = $this->weight_unit_model->deletedata($column);
		if($deleted){
	    	$response['status']="success";
  		}else{
			$response['status']="haserror";
	...
	public function search(){
		...
		$list=$this->weight_unit_model->selectdata("*",array()," ".$cond." ORDER BY id DESC");
		if(count($list)>0){
			foreach($list as $l){
			 $col=array();
			 $col['id']=$l->id;
